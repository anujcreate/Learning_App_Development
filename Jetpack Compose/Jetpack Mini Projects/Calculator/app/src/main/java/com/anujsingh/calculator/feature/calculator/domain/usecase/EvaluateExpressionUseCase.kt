package com.anujsingh.calculator.feature.calculator.domain.usecase
...
class EvaluateExpressionUseCase {
    ...
    operator fun invoke(expression: String): Result<BigDecimal, CalculatorError> {
        ...
        return try {
            var cleanExpr = expression.replace(" ", "").replace(",", "")
            while (cleanExpr.isNotEmpty() && (isOperator(cleanExpr.last()) || cleanExpr.last() == '.')) {
                cleanExpr = cleanExpr.dropLast(1)
            }
            if (cleanExpr.isBlank() || cleanExpr == "-") {
                return Result.Error(CalculatorError.INVALID_EXPRESSION)
            }
